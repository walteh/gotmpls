...
func TestStatusFile(t *testing.T) {
	...
	assert.Len(t, loaded.CoppiedFiles, 1)
	assert.Equal(t, status.CoppiedFiles["test.copy.go"].File, loaded.CoppiedFiles["test.copy.go"].File)
	assert.Equal(t, status.CoppiedFiles["test.copy.go"].Changes, loaded.CoppiedFiles["test.copy.go"].Changes)
}
...
func TestTarballMode(t *testing.T) {
	...
	require.Len(t, status.CoppiedFiles, 1, "should only have one entry in status file")
	entry, ok := status.CoppiedFiles["repo.tar.gz"]
	...
	require.Len(t, status.CoppiedFiles, 1, "should still only have one entry in status file")
}
