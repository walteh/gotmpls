...
// 🔗 source: https://raw.githubusercontent.com/golang/tools/33e624fee8130c31e07a78698cd87013bf884a8d/gopls/internal/protocol/generate/output.go
...
func genCase_old(_ *Model, method string, param, result *Type, dir string) {
	...
	if notNil(result) {
		...
		out.WriteString("\t\t\treturn true, reply_fwd(ctx, conn, r,nil, err)\n")
		out.WriteString("\t\t}\n")
		out.WriteString("\t\treturn true, reply_fwd(ctx, conn, r,resp, nil)\n")
	} else {
